module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        let embed = {
            ...
            title: `${User.user.username} adlı kullanıcının kayıt işlemi engellendi.`,
            description: interaction.message.embeds[0].description,
            fields: [
                { name: `İşlem yapan yönetici:`, value: `<@${interaction.member.user.id}> \`${interaction.member.user.id}\`` },
                { name: `Engellenme sebebi:`, value: reason }
            ]  
        }
        ...
        let content = `${interaction.guild.name} sunucusunda kayıt işleminiz engellendi. \n` + 
                        `Sebep: \`${reason}\``
