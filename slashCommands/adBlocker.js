...
module.exports = {
    ...
    name: "reklam-engel",
    description: "Belirttiğiniz kanala yazılan reklamları engeller veya engeli kaldırır.",
    options: [
        {
            name: "channel",
    ...
    run: async (client, interaction, args) => {
        ...
        let channels = modelfetch?.settings?.adBlocker?.channels
        let _channels = modelfetch?.settings?.adBlocker?.channels?.map(m => `<#${m}>`)

        if (!channel) {
            let text1 = `Sunucuda sırasıyla ${_channels} kanallarında reklamlar engellenmektedir.`
            let text2 = `Sunucuda herhangi bir kanalda reklamlar engellenmemektedir.`
            return await interaction.reply({ content: _channels[0] ? text1 : text2 })
        }

        if (channel?.type !== "GUILD_TEXT")
            return await interaction.reply({ content: `Sunucuda reklamlar yanlızca **metin kanalları**nda engellenebilir.` })
        ...
        if (channelBlock[0]) {
            await Models.guilds.updateOne(
                ...
                { 'settings.adBlocker.channels': orderchannelsBlock ? orderchannelsBlock : [] }
            )
        }
        else {
        ...
        await Models.guilds.updateOne({ guildId: channel?.guildId || interaction?.guildId }, {
            $push: {
                transactions: {
                    ...
                    name: "Reklamları engelleme sisteminde değişiklik yapıldı.",
